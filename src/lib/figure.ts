...
const getParallelArc = (arc: Arc, distance: number): Arc => {
  ...
  const fromDelta = {
    x: Math.cos(Math.atan2(n0.y, n0.x)) * distance,
    y: Math.sin(Math.atan2(n0.y, n0.x)) * -distance,
  };
  const toDelta = {
    x: Math.cos(Math.atan2(n1.y, n1.x)) * distance,
    y: Math.sin(Math.atan2(n1.y, n1.x)) * -distance,
  };

  // ハンドルを法線方向に d 移動させる
  const handleLine: Line = getLine(
    movePoint(arc.p1, fromDelta),
    movePoint(arc.p2, toDelta)
  );
  if (handleLine.type === "vertical") {
    handleLine.x += distance;
  }
  if (handleLine.type === "normal") {
    handleLine.b += Math.sin(Math.atan2(handleLine.a, 1)) * distance;
  }
